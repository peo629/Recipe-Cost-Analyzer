...
export default function Ingredients() {
  ...
  const activeSupplier =
    supplierFilter === ALL_SUPPLIERS_VALUE ? undefined : supplierFilter;
  const listParams = {
    search: debouncedSearch || undefined,
    supplier: activeSupplier,
  };

  const { data: ingredients, isLoading } = useListIngredients(listParams, {
  ...
  const handleDownloadTemplate = () => {
    ...
    const csv = Papa.unparse({
      fields: [...CSV_HEADERS],
      data: [CSV_HEADERS.map((h) => (example as any)[h])],
    });
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      toast.success(
        `Exported ${all.length} ingredient${all.length === 1 ? "" : "s"}`,
      );
    } catch {
      ...
      complete: async (result) => {
        ...
        for (let idx = 0; idx < rows.length; idx++) {
          ...
          if (!parsed.success) {
            ...
            if (errors.length < 3)
              errors.push(
                `Row ${idx + 2}: ${parsed.error.issues[0]?.message ?? "invalid"}`,
              );
            continue;
          }
          try {
            ...
            failed++;
            if (errors.length < 3)
              errors.push(`Row ${idx + 2}: server rejected`);
          }
        }
        queryClient.invalidateQueries({
          queryKey: getListIngredientsQueryKey(),
        ...
        if (failed === 0) {
          ...
          toast.error(
            `Imported ${ok}, skipped ${failed}. ${errors.join(" | ")}`,
          );
        }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Manage your pantry, units, and costs.
          </p>
        </div>
            ...
            <SelectContent>
              {SUPPLIER_FILTER_OPTIONS.map(({ label, value, disabled }) =>
                disabled ? (
                  <div
                    key={value}
                    className="px-2 pt-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/60"
                  >
                    {label}
                  </div>
                ) : (
            ...
            <TableBody>
              {isLoading ? (
                ...
                ingredients?.map((ing) => (
                  <TableRow
                    key={ing.id}
                    data-testid={`row-ingredient-${ing.id}`}
                  >
                    ...
                    <TableCell className="text-right">
                      {ing.purchaseUnitSize} {ing.purchaseUnit}
                    </TableCell>
                    <TableCell className="text-right font-medium">
                      {formatCurrency(ing.purchaseCost)}
                    </TableCell>
                    <TableCell className="text-right">
                      {ing.recipeUnit}
                    </TableCell>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              ...
                              <AlertDialogDescription>
                                Are you sure you want to delete {ing.name}? This
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} placeholder="kg, L, pack" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input {...field} placeholder="g, ml, each" />
                      </FormControl>
                      <p className="text-xs text-muted-foreground mt-1">
                        The unit you typically measure this with in recipes.
                        Cost per unit will be automatically calculated.
                      </p>
                      <FormMessage />
                    </FormItem>
                  )}
